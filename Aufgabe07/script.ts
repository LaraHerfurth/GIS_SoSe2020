namespace Abgabe07 {
    ...
    async function communicateskate(_url: RequestInfo): Promise<void> {
    ...
    async function communicatein(_url: RequestInfo): Promise<void> {
        let response: Response = await fetch(_url);
        console.log("Response", response);
        let produkteJSON: string = await response.text();
        console.log(JSON.parse(produkteJSON)[0].name);
    ...
    function generateskate(skateboards: Produkte[]): void {
        communicateskate("Skateboards.json");
    ...
    async function generatein(inliner: Produkte[]): Promise<void> {
        communicatein("Inliner.json");
